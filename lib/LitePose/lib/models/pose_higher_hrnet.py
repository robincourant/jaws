...
class PoseHigherResolutionNet(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        self.transition1 = self._make_transition_layer(
            [extra["STEM_INPLANES"] * 4], num_channels
        )
    ...
    def _make_final_layers(self, cfg, input_channels):
        ...
        final_layers = []
        output_channels = (
            cfg["MODEL"]["NUM_JOINTS"] + dim_tag
            if cfg["LOSS"]["WITH_AE_LOSS"][0]
            else cfg["MODEL"]["NUM_JOINTS"]
        )
        final_layers.append(
            nn.Conv2d(
