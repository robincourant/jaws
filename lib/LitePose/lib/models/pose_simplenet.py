...
import random
from models.layers.layers import InvBottleneck, convbnrelu, SepConv2d
...
class LitePose(nn.Module):
    def __init__(self, cfg, width_mult=1.0, round_nearest=8, cfg_arch=None):
        ...
        self.deconv_refined, self.deconv_bnrelu = self._make_deconv_layers(
            extra.NUM_DECONV_LAYERS,
            self.filters,
            extra.NUM_DECONV_KERNELS,
        )
        self.final_refined, self.final_channel = self._make_final_layers(
            cfg, self.filters
        )
    ...
    def _make_final_layers(self, cfg, num_filters):
        ...
        return nn.ModuleList(final_refined), final_channel
    ...
    def _make_deconv_layers(self, num_layers, num_filters, num_kernels):
        ...
        return nn.ModuleList(deconv_refined), nn.ModuleList(deconv_bnrelu)
    ...
    def forward(self, x):
        ...
        for i in range(self.num_deconv_layers):
            ...
            input_refined = self.deconv_bnrelu[i](next_input_refined)
            if i > 0:
                final_refined = self.final_refined[i - 1](input_refined)
                final_outputs.append(final_refined)
