...
import random
import time
from models.layers.layers import Bottleneck, convbnrelu, UpConv, FusedMBConv
...
class LitePose(nn.Module):
    def __init__(self, cfg, width_mult=1.0, round_nearest=8):
        ...
        self.first = nn.Sequential(
            ...
            convbnrelu(32, input_channel, ker=7, stride=1)
        )
        ...
        for id_stage in range(len(backbone_setting)):
            ...
            for id_block in range(n):
                ...
                layer.append(FusedMBConv(input_channel, c, s=stride, k=k, r=r))
                input_channel = c
    ...
    def _make_final_layers(self, cfg, num_filters):
        ...
        for i in range(1, extra.NUM_DECONV_LAYERS):
            ...
            final_refined.append(nn.Conv2d(num_filters[i], oup_joint + oup_tag, 3, 1, 1, bias=True))
            final_raw.append(nn.Conv2d(self.channel[-i-3], oup_joint + oup_tag, 3, 1, 1, bias=True))
