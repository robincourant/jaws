...
class SuperLitePose(nn.Module):
    def __init__(self, cfg, width_mult=1.0, round_nearest=8):
        ...
        self.first = nn.Sequential(
            convbnrelu(3, 32, ker=7, stride=2),
            SuperConv2d(32, input_channel, 7, 1, 3, bias=False),
        ...
        for id_stage in range(len(backbone_setting)):
            r, k, c, n, s = backbone_setting[id_stage]
            c = _make_divisible(c * width_mult, round_nearest)
            layer = []
            for id_block in range(n):
                stride = s if id_block == 0 else 1
                layer.append(SuperFusedMBConv(input_channel, c, s=stride, k=k, r=r))
                input_channel = c
        ...
        self.filters = extra["NUM_DECONV_FILTERS"]
        ...
        (
            ...
            self.final_raw,
            self.final_channel,
        ) = self._make_final_layers(cfg, self.filters)
    ...
    def _make_final_layers(self, cfg, num_filters):
        ...
        for i in range(1, extra.NUM_DECONV_LAYERS):
            ...
            final_refined.append(
                SuperConv2d(num_filters[i], oup_joint + oup_tag, 3, 1, 1, bias=True)
            )
            final_raw.append(
                SuperConv2d(
                    self.channel[-i - 3], oup_joint + oup_tag, 3, 1, 1, bias=True
                )
            )
    ...
    def forward(self, x):
        ...
        x = self.first[0](x)
        x = self.first[1](x, cfg_arch["input_channel"])
        x = self.first[2](x)
        x = self.first[3](x)
        x_list = [x]
        ...
        for id_stage in range(len(self.stage)):
            ...
            for id_block in range(n):
                ...
                # For ResNet space, only search channls
                tmp = self.stage[id_stage][id_block](tmp, c, t)
            x_list.append(tmp)
        final_outputs = []
