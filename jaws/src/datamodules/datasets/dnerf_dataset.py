...
class DNeRFDataset:
    def __init__(
        ...
        if self.mode == "colmap":
            with open(os.path.join(self.root_path, "d_transforms.json"), "r") as f:
            ...
            # only load one specified split
                ...
                with open(
                    os.path.join(self.root_path, f"d_transforms_{type}.json"),
                    "r",
                ) as f:
                    transform = json.load(f)
            elif type == "infer":
                print("infer mode")
                with open(
                    os.path.join(self.root_path, "d_transforms_train.json"),
                    "r",
                ) as f:
                    transform = json.load(f)
        ...
        if self.mode == "colmap" and type == "test":
            ...
            time0 = (
                f0["time"]
                if "time" in f0
                else int(os.path.basename(f0["file_path"])[:-4])
            )
            time1 = (
                f1["time"]
                if "time" in f1
                else int(os.path.basename(f1["file_path"])[:-4])
            )
        ...
        self.times = torch.from_numpy(np.asarray(self.times, dtype=np.float32)).view(
            -1, 1
        )  # [N, 1]
